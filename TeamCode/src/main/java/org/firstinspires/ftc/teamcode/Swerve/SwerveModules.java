...
public class SwerveModules extends SwerveModuleBase {
    ...
    private final DcMotorEx rawDriveEncoder; //digital
    ...
    public SwerveModules(String id, Translation2d translationOffset, Rotation2d angleOffset, String turnName, String driveName, String turnEncoderName, String driveEncoderName) {
        super(id, translationOffset);

        this.angleOffset = angleOffset;

        driveMotor = hardwareMap.get(DcMotorEx.class, driveName);
        rawDriveEncoder = hardwareMap.get(DcMotorEx.class, driveEncoderName);

        turnMotor = hardwareMap.get(CRServo.class, turnName);
        turnEncoder = hardwareMap.get(AnalogInput.class, turnEncoderName);

        realDriveEncoder = new EncoderConversion(rawDriveEncoder, 0, 0, 0);

        driveControllerPID = new PIDController(0, 0, 0);
